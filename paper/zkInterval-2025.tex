...
\begin{abstract}
In this paper, we introduce zkInterval, a novel cryptographic range-proof protocol providing constant-size range proofs that a secret value lies within a public interval without disclosing the secret value or requiring a trusted setup. By leveraging Pedersen commitments, Weil pairings, and Schnorr $\Sigma$-protocols, zkInterval offers a practical, trustless, and confidential solution while maintaining proof sizes independent of the interval length. Compared to existing range-proofs, zkInterval significantly reduces complexity, storage, and overhead, making zkInterval particularly well-suited for privacy-focused applications and scenarios with variable interval lengths. This work addresses a gap in contemporary range-proof techniques, specifically the existence of a practical, transparent, and constant-sized range-proof protocol. ZkInterval fills this gap by offering a robust solution to a critical challenge in modern cryptography.
\end{abstract}
...
Cryptographic range proofs demonstrate that a secret value lies within a public interval without revealing it. They are foundational for privacy-focused applications such as confidential transactions, secure multiparty computations, and private data verification. Finding a range-proof protocol that is confidential, trustless, practical, and efficient poses a significant challenge. Many existing methods generate proofs whose size scales with the interval's range, which can severely limit their efficiency, especially in applications that demand a constant proof size across variable interval lengths. Some methods provide constant-size proof but require trusted setups, which can introduce vulnerabilities such as collusion-based attacks. Methods that provide a transparent setup and constant-sized proofs are impractical, utilize less used curves, or have relaxed security assumptions \cite{christetal2024}.

This paper addresses these limitations by introducing zkInterval, a range-proof protocol that produces a practical, trustless, confidential, and constant-size proof independent of the interval's range. By integrating Pedersen commitments \cite{pedersen1991}, Weil pairings \cite[pp.62--63]{menezes1993}, and Schnorr $\Sigma$-protocols \cite[pp.310--311]{thaler2022}, zkInterval securely demonstrates that a secret value lies within a publicly known interval while keeping the value itself hidden. This integration results in a robust protocol whose proof sizes remain invariant, regardless of interval length, significantly enhancing efficiency and scalability compared to current state-of-the-art solutions like Bulletproofs \cite{bunz2018bulletproofs}. While Bulletproofs have reduced proof sizes to sublinear scales, all while being trustless, their proof size still depends on the interval's range, which can be a limiting factor in applications with variable interval lengths.

By providing practical, constant-sized, trustless, and confidential proofs, zkInterval directly fills a gap in existing cryptographic range-proof techniques. It is uniquely suited for modern privacy-focused applications requiring scalable and secure solutions.

The remainder of the paper is as follows: Section 2 presents the derivation of the zkInterval protocol, detailing its mathematical foundation and the integration of Pedersen commitments, Weil pairings, and Schnorr $\Sigma$-protocols. Section 3 rigorously examines the protocol's security properties, establishing its completeness, soundness, and zero-knowledge guarantees. Finally, Section 4 discusses the practical implementation of zkInterval on the Cardano blockchain, highlighting its performance benefits and suitability for privacy-focused and scalable cryptographic applications.
...
\begin{namedproof}[Proof of Soundness]
...
\begin{namedproof}[Proof of Zero-Knowledge]
