...
Each proof demostrates that the zkInterval protocol is a zero-knowledge non-interactive proof system.

The zkInterval protocol is fairly succinct. When implemented on the Cardano blockchain, the resulting proof size is 546 bytes for a 64-bit range which is smaller than Bulletproofs in both the 64-bit and 32-bit cases. See Table 4 in \cite{christetal2024}. For the 64-bit case the average prover time is 90ms and verifier time is 2.23ms. The proof generation is slow due to the benchmark being ran with python using the \texttt{py\_ecc} module for the BLS12-381 curve which is not heavily optmized for performance. The prover time could be decreased significantly with optimal C bindings via rust which would make is comparable to other protocols. The verification is done with the Aiken VM and represents the upperbound on the real computation time required to verify the proof.
